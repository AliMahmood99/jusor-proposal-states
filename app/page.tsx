...
const ClockIcon = ({ size = 12, color = "#717171" }: { size?: number; color?: string }) => <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2"><circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/></svg>;
...
const ChevronDown = ({ open }: { open: boolean }) => <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#8f8f8f" strokeWidth="2" strokeLinecap="round" style={{ transition: "transform 0.2s", transform: open ? "rotate(180deg)" : "rotate(0)" }}><path d="M6 9l6 6 6-6"/></svg>;
...
const ShieldIcon = ({ size = 12, color = "#4446d2" }: { size?: number; color?: string }) => <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="2" strokeLinecap="round"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/></svg>;
...
const PaymentBreakdown = ({ baseAmount = 250 }: { baseAmount?: number }) => {
  ...
  return (
    <div style={{ background: "#f8f9fb", borderRadius: 12, overflow: "hidden", border: "1px solid #eef0f2" }}>
...
const CircularTimer = ({ hoursLeft = 58, totalHours = 72 }: { hoursLeft?: number; totalHours?: number }) => {
...
const BottomBar = ({ buttons }: { buttons: React.ReactNode }) => (
...
const BtnReject = ({ label = "Reject" }: { label?: string }) => (
...
const BtnAccept = ({ label = "Accept" }: { label?: string }) => (
...
const ProfileCard = ({ badge, dimmed = false }: { badge: React.ReactNode; dimmed?: boolean }) => (
...
const TimelineInfo = ({ rows }: { rows?: [string, string, string?][] }) => {
    ...
    <div style={{ background: "white", border: "1px solid #d5d7d8", borderRadius: 14, padding: 12, boxShadow: "0 2px 8.6px rgba(0,0,0,0.08)", display: "flex", flexDirection: "column", gap: 10 }}>
      ...
      <div style={{ background: "#ff4f45", borderRadius: 20, padding: "12px 16px", color: "white" }}>
        ...
        <div style={{ background: "rgba(255,255,255,0.15)", borderRadius: 50, padding: "5px 12px", display: "flex", alignItems: "center", justifyContent: "center", gap: 6, backdropFilter: "blur(4px)" }}>
          <span style={{ width: 7, height: 7, borderRadius: "50%", background: "white", animation: "blink 1.5s infinite", display: "inline-block" }} />
  ...
  <PhoneFrame bottomButtons={<BtnAccept label="Send New Offer" />}>
    <div style={{ background: "white", border: "1px solid #d5d7d8", borderRadius: 14, padding: 12, boxShadow: "0 2px 8.6px rgba(0,0,0,0.08)", display: "flex", flexDirection: "column", gap: 10 }}>
      ...
      <div style={{ background: "#ff4f45", borderRadius: 20, padding: "12px 16px", color: "white", opacity: 0.6 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
          <span style={{ fontSize: 16, fontWeight: 500, textDecoration: "line-through", textDecorationColor: "rgba(255,255,255,0.6)" }}>Proposed Price</span>
          <span style={{ fontSize: 16, fontWeight: 600, textDecoration: "line-through", textDecorationColor: "rgba(255,255,255,0.6)" }}>SAR 180</span>
        </div>
